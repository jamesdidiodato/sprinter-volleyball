...
- **Season Standings (player W/L)**: Only round robin games count toward player seasonWins/seasonLosses. Playoff games (semis/finals) are excluded so that all teams contribute equal game counts to standings regardless of playoff performance. This prevents unfair inequality for leagues with 5+ teams where not all teams make playoffs.
